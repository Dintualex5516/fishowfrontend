...
//       if (e.key === 'ArrowDown') {
//         if (editData && currentItemIndex < editData.items.length - 1) {
//           targetItemId = editData.items[currentItemIndex + 1].id;
//         }
...
//           `[data-item-id="${targetItemId}"][data-field="${field}"] input`
//         );
//         if (nextWrapperInput) {
...
//           const nextInput = document.querySelector<HTMLInputElement>(
//             `input[data-item-id="${targetItemId}"][data-field="${field}"]`
//           );
//           nextInput?.focus();
//         }
//       }
...
//       // If modal is open, don't handle keys for the list
//       if (!!editingEntry || !!deletingEntry || !!selectedEntry || !!notification) return;
//       if (currentEntries.length === 0) return;

//       if (e.key === 'ArrowDown') {
//         e.preventDefault();
//         setFocusedIndex(prev => Math.min(prev + 1, currentEntries.length - 1));
//       } else if (e.key === 'ArrowUp') {
//         e.preventDefault();
//         setFocusedIndex(prev => Math.max(prev - 1, 0));
//       } else if (e.key === 'Enter') {
//         if (focusedIndex >= 0 && currentEntries[focusedIndex]) {
//           viewDetails(currentEntries[focusedIndex]);
//         }
//       }
//     };
//     window.addEventListener('keydown', handleKeyDown);
//     return () => window.removeEventListener('keydown', handleKeyDown);
//   }, [currentEntries, editingEntry, deletingEntry, selectedEntry, notification, focusedIndex]);

//   useEffect(() => {
//     if (focusedIndex >= 0) {
//       const row = document.getElementById(`sales-summary-row-${focusedIndex}`);
...
//                   <button
//                     onClick={() => {
//                       const printContents = document.getElementById("sales-entry-details-content")?.innerHTML;
//                       if (printContents) {
//                         const printWindow = window.open("", "", "height=600,width=800");
//                         if (printWindow) {
//                           const currentDate = new Date().toLocaleDateString('en-GB').replace(/\//g, '-');
//                           printWindow.document.title = `Sales Entry Details - ${currentDate}`;
//                           printWindow.document.write('<html><head><title>Fishow - Sales Entry Details</title>');
//                           printWindow.document.write(
//                             '<style>body{font-family: Arial, sans-serif; margin: 20px;} h1{text-align:center;} .header-grid {display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 20px;} .label {font-weight: bold; font-size: 12px; color: #555;} .value {font-size: 14px;} table {width: 100%; border-collapse: collapse; margin-top: 20px;} th, td {border: 1px solid #000; padding: 8px; text-align: left; font-size: 12px;} th {background-color: #f2f2f2;} .footer {margin-top: 20px; text-align: center; font-size: 12px;}</style>'
//                           );
//                           printWindow.document.write("</head><body>");
//                           printWindow.document.write("<h1>Fishow - Sales Entry</h1>");

//                           // Construct the content manually or use innerHTML. 
//                           // Using innerHTML is easiest but requires the modal content to be clean HTML.
//                           // Let's manually construct header part to ensure grid layout in print.

//                           printWindow.document.write(`
//                             <div class="header-grid">
//                               <div><div class="label">Date</div><div class="value">${new Date(selectedEntry.date).toLocaleDateString('en-GB').replace(/\//g, '-')}</div></div>
//                               <div><div class="label">Party</div><div class="value">${selectedEntry.party}</div></div>
//                               <div><div class="label">Total Box</div><div class="value">${selectedEntry.totalBox}</div></div>
//                               <div><div class="label">Salesman</div><div class="value">${selectedEntry.salesman}</div></div>
//                             </div>
//                           `);

//                           // Now write the table
//                           printWindow.document.write('<table><thead><tr><th>Customer</th><th>Item</th><th>Box</th><th>KG</th><th>Price</th><th>Total</th></tr></thead><tbody>');
...
//               <div id="sales-entry-details-content" className="p-4 sm:p-6 space-y-4 sm:space-y-6">
//                 {/* Header Information */}
//                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
//                   <div>
//                     <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 sm:mb-2">
