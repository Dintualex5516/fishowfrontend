...
//   return res.data;
// }
// 📌 CLEANED TYPES — match backend after removing product/price/total/kg

export type BoxItem = {
  id: string | number;
  customer: string | null;   // customer name or ID mapped from FE
  box: number;               // box count
  box_type?: string | null;  // party_id used as box type
  created_at?: string | null;
};
...
// UPDATE — payload rewritten to match new backend structure
export async function updateBoxSaleRow(id: string | number, payload: Record<string, any>) {
  const res = await api.put(`/api/box-sales/${encodeURIComponent(String(id))}`, payload);
  return res.data;
}
